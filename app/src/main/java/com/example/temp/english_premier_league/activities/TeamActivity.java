...
public class TeamActivity extends AppCompatActivity {
    ...
    public void init(Team item) {
        mBigCrest = findViewById(R.id.big_crest);
        mFixtures = findViewById(R.id.fixture_textView);
        mPlayers = findViewById(R.id.players_textView);
        mTeamName = findViewById(R.id.team_name_textView);
        mShortName = findViewById(R.id.short_name_textView);
        mCode = findViewById(R.id.team_code_textView);
        mSquadMV = findViewById(R.id.squad_market_value);
        displayImage(item.getCrestUrl(), mBigCrest, this);

        mTeamName.setText(String.format("Team:    %s", item.getTeamName()));
        mShortName.setText(String.format("Short:  %s", item.getTeamShortName()));
        mCode.setText(String.format("Code:    %s", item.getTeamCode()));
        mCompetitionName = getIntent().getStringExtra("ActionBarTitle");

        if (item.getSquadMarketValue().equals("null")) {
            mSquadMV.setText("");
        } else {
            mSquadMV.setText(String.format("Squad Value:  %s", item.getSquadMarketValue()));
        }
    }
